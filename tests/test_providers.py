...
class TestBaremetal:
    def test_deploy_sh_flow(self, mock_conn):
        def mock_run(cmd, **kwargs):
            if "test -f deploy.sh" in cmd: return Mock(ok=True)
    ...
    def test_makefile_fallback(self, mock_conn, monkeypatch):
        ...
        def mock_run(cmd, **kwargs):
            if "test -f deploy.sh" in cmd: return Mock(ok=False)
            if "test -f Makefile" in cmd: return Mock(ok=True)
            return Mock(ok=False)
        mock_conn.run.side_effect = mock_run
...
class TestDocker:
    ...
    def test_deploy_docker_with_profile(self, mock_conn, monkeypatch):
        ...
        with patch("src.providers.docker.docker_login"), \
             patch("src.providers.docker.run_command") as mock_run:
