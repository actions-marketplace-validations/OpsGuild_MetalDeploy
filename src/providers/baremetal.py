...
def deploy_baremetal(conn):
    ...
    with conn.cd(config.GIT_SUBDIR):
        ...
        if deploy_script_check.ok:
            ...
            result = run_command(conn, "./deploy.sh; EXIT_CODE=$?; if [ $EXIT_CODE -ne 0 ]; then echo 'Command failed with exit code:' $EXIT_CODE; exit $EXIT_CODE; fi")
            ...
            if makefile_check.ok:
                ...
                result = run_command(conn, f"make {config.ENVIRONMENT}; EXIT_CODE=$?; if [ $EXIT_CODE -ne 0 ]; then echo 'Command failed with exit code:' $EXIT_CODE; exit $EXIT_CODE; fi")
                if "Command failed with exit code:" in result.stdout:
                    for line in result.stdout.split("\n"):
                        if "Command failed with exit code:" in line:
                            exit_code = line.split("exit code:")[-1].strip()
                            raise ValueError(f"make {config.ENVIRONMENT} failed with exit code: {exit_code}")
            else:
                raise ValueError("No deploy_command specified and no deploy.sh or Makefile found. Please specify deploy_command input.")
